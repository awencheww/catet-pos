    ...
    <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4 p-3 mb-5">
      <div class="container-fluid">
        ...
        <div class="row g-3">
          ...
          <form action="{{route('products.update', $product)}}" method="POST" class="row g-3" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="row g-3">
              @foreach ($fields as $field)
                        ...
                        <select class="form-select" name="{{$field}}" id="{{$field}}" aria-label="Select Category">
                            @if ($suppliers)
                              @foreach ($suppliers as $supplier)
                                <option value="{{$supplier->id}}" {{$product->supplier_id == $supplier->id ? 'selected' : ''}}>{{ blank($supplier->company) ? $supplier->contact_name : $supplier->company }}</option>
                              @endforeach
                            @endif
                        </select>
                        @error($field) <span class="invalid-feedback">{{$message}}</span> @enderror
                  ...
                  @elseif ($field == 'expiry')
                    <div class="form-group mb-1 col-md-4">
                        <label for="{{ $field }}" class="mb-1">{{ ucwords(str_replace('_', ' ', $field)) }}</label>
                        <input type="date" name="{{ $field }}" id="{{ $field }}" value="{{ old($field, $product) }}" class="form-control  @error($field) is-invalid @enderror">
                        @error($field) <span class="invalid-feedback">{{$message}}</span> @enderror
                    </div>
                  ...
                  @elseif ($field == 'unit_price')
                    <div class="row g-2">
                      <div class="form-group mb-1 col-md-3">
                        <label for="{{ $field }}" class="mb-1">{{ ucwords(str_replace('_', ' ', $field)) }}</label>
                        <input type="number" min="1" name="{{ $field }}" id="{{ $field }}" value="{{ old($field, $product) }}" class="form-control  @error($field) is-invalid @enderror">
                        @error($field) <span class="invalid-feedback">{{$message}}</span> @enderror
                    </div>
                    </div>
                  ...
                  @elseif ($field == 'total_cost')
                    <div class="form-group mb-1 col-md-3">
                        <label for="{{ $field }}" class="mb-1">{{ ucwords(str_replace('_', ' ', $field)) }}</label>
                        <input type="number" min="1" readonly name="{{ $field }}" id="{{ $field }}" value="{{ old($field, $product) }}" class="form-control  @error($field) is-invalid @enderror">
                        @error($field) <span class="invalid-feedback">{{$message}}</span> @enderror
                    </div>
                  ...
                  @else 
                    <div class="form-group mb-1 col-md-4">
                        <label for="{{ $field }}" class="mb-1">{{ ucwords(str_replace('_', ' ', $field)) }}</label>
                        <input type="text" name="{{ $field }}" id="{{ $field }}" value="{{ old($field, $product) }}" class="form-control  @error($field) is-invalid @enderror">
                        @error($field) <span class="invalid-feedback">{{$message}}</span> @enderror
                    </div>
                  @endif
